...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderPortfolioInputs() {
      ...
      card.innerHTML =
        ...
        '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">' +
          ...
          '<div class="form-group">' +
            ...
            '<input type="text" class="admin-input port-cat-input" data-idx="' + idx + '" value="' + escapeAttr(item.category) + '" placeholder="Longform / Shortform" />' +
          '</div>' +
        '</div>' +
        '<div class="form-group">' +
    ...
    container.querySelectorAll('.port-cat-input').forEach(function (input) {
      input.addEventListener('input', function () { currentPortfolio[this.getAttribute('data-idx')].category = this.value; });
    });
